    ...
    for (const suit of suits) {
        retval.push(
            <div key={suit} className='flex relative items-center justify-center'>
                ...
                <input disabled={firstRound && trumpSuit !== suit} defaultChecked={suit === trumpSuit} type='radio' name='suit' value={suit} className={`appearance-none cursor-pointer border rounded w-24 h-8 ${buttonSvg} checked:dark:bg-yellow-500`} />
            </div>);
    }
    return (
        <div ref={ref} {...rest} className={`flex flex-col gap-2 ${firstRound ? "col-span-1" : "col-span-2"}`}>
            {retval}
        </div>);
}