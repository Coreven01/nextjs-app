...
import { MouseEventHandler, use, useCallback, useState } from "react";
...
export default function useTileClick(
     ...
     */
    const handleTileClick = useCallback((event: React.MouseEvent<HTMLButtonElement, MouseEvent>, row: number, column: number) => {
    ...
    }, [shouldHandleMouseClick, mapState, gameState, startTimer, onPlay, handledMouseClick]);

    /**
     * Flag the tile as a bomb. Clears the tile if already flagged.
     */
    const handleTileRightClick = useCallback((event: React.MouseEvent<HTMLButtonElement, MouseEvent>, row: number, column: number) => {
    ...
    }, [shouldHandleMouseRightClick, mapState, gameState, startTimer, onPlay, handledMouseRightClick]);

    /**
     * Prevent from clicking the tile if the mouse cursor exits the button area.
     */
    const handleTileMouseLeave = useCallback((event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
     ...
     */
    const handleTileMouseUp = useCallback((event: React.MouseEvent<HTMLButtonElement, MouseEvent>, row: number, column: number) => {
        ...
        if (shouldHandleDoubleMouseUp && doubleMouseDownEvents.length > 1) {
            ...
            if (bombTileValue) {
                const newExposedMap = validateAndClickAjacentTiles(bombTileValue, row, column, gameState, mapState);
                if (newExposedMap)
        ...
        popMouseDownEvent();
        setAdjacentTiles([]);
    }, [shouldHandleDoubleMouseUp, doubleMouseDownEvents, mapState, gameState, 
        onPlay, handledMouseClick, handleDoubleMouseUp, handledMouseRightClick, popMouseDownEvent]);

    const addMouseDownEvent: MouseEventHandler<HTMLButtonElement> = useCallback((event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
     ...
     */
    const handleTileMouseDown = useCallback((event: React.MouseEvent<HTMLButtonElement, MouseEvent>, row: number, column: number) => {
        ...
        if (doubleMouseDownEvents.length >= 1) {
            ...
            if (exposedTileValue >= 1 && exposedTileValue <= 8) {
                ...
                getDirectAdjacentTiles(row, column, gameState, mapState)
                    .map(tile => {
    ...
    }, [doubleMouseDownEvents, gameState, mapState, addMouseDownEvent, handleDoubleMouseDown]);
