...
const useGamePlayLogic = () => {
  ...
  const { determineCurrentWinnerForTrick, getRandomScoreForDifficulty } = useGameData();
  const {
    cardIsRightBower,
    ...
    const winningCards = cardsAvailableToPlay.filter((c) => {
      return (
        ...
        (c.value > winningCardVal &&
          (c.card.suit === trumpCard.suit || cardIsLeftBower(c.card, trumpCard))) ||
  ...
  function determineCardToPlay(game: EuchreGameInstance, difficulty: GameDifficulty): Card {
    ...
    let cardToPlay: Card;
    const randomChanceForDifficulty = getRandomScoreForDifficulty(
...
export default useGamePlayLogic;
