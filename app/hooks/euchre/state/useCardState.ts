...
const useCardState = (
  ...
  const handleResetHandState = useCallback(() => {
    logConsole('[CARD STATE] [handleResetHandState]', ' player: ', player.name);
    resetForNewHand();
  }, [player.name, resetForNewHand]);

  const handleCreateHandState = useCallback(() => {
    logConsole('[CARD STATE] [handleCreateHandState]', ' player: ', player.name);
    addInitializeHandStateEvent(state, eventHandlers, player);
    setInitialPlayerHandState();
  }, [eventHandlers, player, setInitialPlayerHandState, state]);

  const handleInitializeCardState = useCallback(() => {
    addInitializeCardStateEvent(state, eventHandlers, player);

    logConsole('[CARD STATE] [handleInitializeCardState]', ' player: ', player.name);
    initializeSortOrder();
    setInitialCardStates();
  }, [eventHandlers, initializeSortOrder, player, setInitialCardStates, state]);
