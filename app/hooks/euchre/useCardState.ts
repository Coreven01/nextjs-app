...
const useCardState = (
  ...
  useEffect(() => {
    const beginRegroupCards = async () => {
      const shouldRegroupCards = !cardsRefBeginSet.current && handState && cardStates.length == 5;
      if (shouldRegroupCards) {
        ...
        const delay = state.euchreSettings.gameSpeed * player.team;
