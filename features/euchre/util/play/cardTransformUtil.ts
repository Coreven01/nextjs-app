...
const createCardAnimationState = (index: number): CardAnimationState => {
  return {
    cardIndex: index,
...
const getSpringToMoveToPlayer = (
  ...
  if (useTransition) {
    const variation = delayVariation ?? 'none';
    const speed = tempSpringContect.gameSpeed ?? 1200;
    const duration = getDurationSeconds(speed);
    const maxVariationDuration = duration / 2;

    let delayBetweenMove = 0;
    let variationDivisor = 0;

    switch (variation) {
      case 'low':
        variationDivisor = 3;
        delayBetweenMove = maxVariationDuration / variationDivisor;
        break;
      case 'med':
        variationDivisor = 6;
        delayBetweenMove = maxVariationDuration / variationDivisor;
